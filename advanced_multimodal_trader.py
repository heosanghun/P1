...
class AdvancedMultimodalTrader(BasicTrader):
    ...
    def __init__(self, candlestick_analyzer, sentiment_analyzer, config=None):
        ...
        self.logger = logging.getLogger('AdvancedMultimodalTrader')
        ...
        # 초기 잔고 설정
        self.initial_balance = self.config.get('trading', {}).get('initial_balance', 10000.0)
        
        # 상태값 초기화
        self.current_cash = self.config.get('initial_capital', 10000.0)  # 초기 자본금을 0에서 10000으로 변경
        self.portfolio_value = self.current_cash  # 포트폴리오 가치 = 현금 + 보유 자산가치
        self.current_positions = {}  # 현재 포지션 {symbol: quantity}
        self.portfolio_value_history = []  # 포트폴리오 가치 히스토리
        self.position_history = []  # 포지션 히스토리
        self.transaction_history = []  # 거래 히스토리
        self.signals_history = []  # 트레이딩 신호 히스토리
    
    def set_progress_callback(self, callback_fn):
        """진행 상황을 업데이트할 콜백 함수 설정"""
        self.progress_callback = callback_fn
        self.logger.info("진행 상황 콜백 함수 설정 완료")
    
    def _setup_advanced_rl(self):
        """개선된 강화학습 설정"""
        ...
        # 멀티모달 상태 벡터 확장
        # 기존: 패턴 신호, 감성 신호, 가격 변화, 포지션(3), 포트폴리오 가치
        # 상태 차원을 기존 rl_trader.py와 일치시킴 (7)
        state_dim = 7  # 이전 10에서 7로 수정
        action_dim = 3
        ...
        self.logger.info("고급 강화학습(PPO) 거래 모듈 초기화 완료")
    ...
    def generate_expert_systems_metrics(self):
        ...
        try:
            ...
            # 5. 샤프 비율
            risk_free_rate = 0.02 / 365  # 연간 2% 무위험 수익률 가정
    ...
    def _get_advanced_action(self, pattern_signals, sentiment_signals, price_change, current_position, current_value, date):
        """
        고급 행동 결정
        - 앙상블 모델의 결정을 조합하여 최종 행동 결정
        - 각 모델의 가중치는 이전 성능에 따라 동적으로 조정
        ...
        try:
            ...
            if self.use_rl:
                try:
                    ...
                    rl_action, _ = self.rl_trader.get_action(state, training=False)
                except Exception as e:
                    self.logger.warning(f"RL 모델 예측 중 오류: {str(e)}, 기본 액션 사용")
                    rl_action = 2  # 기본값: 현금 보유
            else:
                rl_action = 2  # 기본값: 현금 보유
            
            # 융합 모델 (패턴+감성+가격)
            try:
                fusion_action = self._get_fusion_action(pattern_signals, sentiment_signals, price_change, current_position)
            except Exception as e:
                self.logger.warning(f"융합 모델 예측 중 오류: {str(e)}, 기본 액션 사용")
                fusion_action = 2  # 기본값: 현금 보유
            
            # 기술적 분석 기반 행동
            try:
                technical_action = self._get_technical_action(pattern_signals, price_change)
            except Exception as e:
                self.logger.warning(f"기술 모델 예측 중 오류: {str(e)}, 기본 액션 사용")
                technical_action = 2  # 기본값: 현금 보유
            
            # 앙상블 방법론으로 최종 결정
            if self.use_ensemble and self.model_ensemble is not None:
                predictions = {
                    'rl': rl_action,
                    'fusion': fusion_action,
                    'technical': technical_action
                }
                
                try:
                    final_action = self.model_ensemble.predict(predictions)
                except AttributeError:
                    # predict 메서드가 없으면 combine_predictions 메서드 사용
                    try:
                        final_action = self.model_ensemble.combine_predictions(predictions)
                    except Exception as e:
                        self.logger.warning(f"앙상블 예측 중 오류: {str(e)}, 가중 평균 사용")
                        # 직접 가중 평균 계산
                        weights = {'rl': 0.3, 'fusion': 0.5, 'technical': 0.2}
                        final_action_float = sum(weights[model] * action for model, action in predictions.items())
                        final_action = int(round(final_action_float))
                except Exception as e:
                    self.logger.warning(f"앙상블 predict 메서드 호출 중 오류: {str(e)}, 기본 액션 사용")
                    final_action = fusion_action  # 기본값으로 융합 모델 사용
                
                # 성능 히스토리 업데이트 (다음번 가중치 조정에 사용)
                if self.dynamic_weights and hasattr(self, 'performance_history'):
                    try:
                        # 각 모델의 성능 추적
                        for model_name, action in predictions.items():
                            # 가상의 결과로 성능 평가
                            if action == 0 and price_change > 0:  # 매수 성공
                                self.performance_history[model_name].append(1.0)
                            elif action == 1 and price_change < 0:  # 매도 성공
                                self.performance_history[model_name].append(1.0)
                            elif action == 2:  # 홀딩에는 작은 페널티
                                self.performance_history[model_name].append(0.5)
                            else:  # 틀린 예측
                                self.performance_history[model_name].append(0.0)
                        ...
                        # 성능 기반 가중치 업데이트 (10 거래일마다)
                        if len(self.performance_history.get('rl', [])) >= max_history:
                            # 성능 평균 계산
                            perf = {
                                model: np.mean(scores) 
                                for model, scores in self.performance_history.items()
                                if scores  # 빈 리스트가 아닌 경우에만
                            }
                            ...
                            if total_perf > 0:
                                weights = {
                                    model: score / total_perf 
                                    for model, score in perf.items()
                                }
                                
                                # 모델 앙상블 가중치 업데이트
                                try:
                                    self.model_ensemble.set_weights(weights)
                                except AttributeError:
                                    # set_weights 메서드가 없으면 weights 속성 직접 업데이트
                                    self.model_ensemble.weights = list(weights.values())
                                except Exception as e:
                                    self.logger.warning(f"가중치 업데이트 중 오류: {str(e)}")
                                
                                # 히스토리 초기화
                                for model in self.performance_history:
                                    self.performance_history[model] = []
                    except Exception as e:
                        self.logger.warning(f"성능 히스토리 업데이트 중 오류: {str(e)}")
            else:
                # 앙상블 없이 기본 융합 모델 사용
                final_action = fusion_action
            
            # 액션 타입 및 값 확인
            if final_action is None:
                self.logger.warning("액션이 None입니다. 기본 액션(2: 홀딩)으로 설정합니다.")
                final_action = 2
            elif not isinstance(final_action, int):
                try:
                    final_action = int(final_action)
                except:
                    self.logger.warning(f"액션이 정수가 아닙니다: {type(final_action)}. 기본 액션(2: 홀딩)으로 설정합니다.")
                    final_action = 2
            
            # 유효 범위 확인 (0, 1, 2 중 하나여야 함)
            if final_action not in [0, 1, 2]:
                self.logger.warning(f"액션이 유효하지 않습니다: {final_action}. 기본 액션(2: 홀딩)으로 설정합니다.")
                final_action = 2
                
            return final_action
        
        except Exception as e:
            self.logger.error(f"행동 결정 중 예상치 못한 오류 발생: {str(e)}")
            import traceback
            self.logger.error(traceback.format_exc())
            return 2  # 오류 발생 시 기본 행동: 홀딩

    def _get_technical_action(self, pattern_signal, price_change):
        """기술적 분석 기반 행동 결정"""
        # 패턴 신호와 가격 변화를 조합한 단순한 규칙 기반 결정
        if pattern_signal > 0.3:
            return 0  # 매수
        elif pattern_signal < -0.3:
            return 1  # 매도
        else:
            # 약한 신호에서는 가격 변화 추세 따름
    ...
    def _get_fusion_action(self, pattern_signal, sentiment_signal, price_change, current_position):
        """융합 모델 기반 행동 결정"""
        # 신호 융합 (가중치 적용)
        pattern_weight = 0.5
        ...
        combined_signal = (
            ...
            price_weight * (1.0 if price_change > 0 else -1.0 if price_change < 0 else 0)
        )
        
        # 포지션 유지에 대한 바이어스 (빈번한 거래 방지)
        position_bias = 0.1
        
        # 현재 포지션에 따른 조정
        if current_position == 'long':
            combined_signal += position_bias
    ...
    def evaluate_performance(self):
        ...
        try:
            ...
            # 9. 베타 (시장 대비 변동성) - 가상의 시장 수익률 사용
            # SVD 오류 방지를 위한 안정화 조치
            try:
                # 실제로는 BTC/USD 인덱스와 비교해야 함
                mock_market_returns = np.random.normal(0.0005, 0.02, len(daily_returns))  # 가상의 시장 일일 수익률
                if len(daily_returns) > 0 and len(mock_market_returns) > 0:
                    # 데이터 안정화
                    daily_returns_np = np.array(daily_returns)
                    # 이상치 제거
                    daily_returns_np = np.clip(daily_returns_np, -0.1, 0.1)
                    
                    # 공분산 계산 시 안정화 옵션 추가
                    try:
                        covariance = np.cov(daily_returns_np, mock_market_returns)[0][1]
                        market_variance = np.var(mock_market_returns)
                ...
                self.logger.warning(f"베타 계산 중 오류 발생: {str(e)}")
                metrics['beta'] = 1.0
            
            # 10. 알파 (초과 수익률)
            try:
                market_avg_return = np.mean(mock_market_returns) if 'mock_market_returns' in locals() and mock_market_returns.size > 0 else 0
                alpha = avg_daily_return - (risk_free_rate + beta * (market_avg_return - risk_free_rate))
                metrics['alpha'] = alpha * 100 * 252  # 연환산 및 % 변환
            except Exception as e:
                self.logger.warning(f"알파 계산 중 오류 발생: {str(e)}")
                metrics['alpha'] = 0.0
    ...
    def process_trading_day(self, date, data, price, position, portfolio_value):
        ...
        try:
            ...
            if 'prev_price' in data and data['prev_price'] > 0.000001:  # 작은 임계값 추가
                ...
                price_change = 0
                self.logger.warning(f"거래일 {date}에서 이전 가격이 0에 가까워 가격 변화율을 0으로 설정합니다.")
            ...
            if new_portfolio_value < 0:
                self.logger.warning(f"거래일 {date}에서 포트폴리오 가치가 음수({new_portfolio_value:.2f})입니다. 0으로 보정합니다.")
    ...
    def calculate_sharpe_ratio(self):
        ...
        # 무위험 수익률 (연간 2% 가정)
        risk_free_rate = 0.02 / 252  # 일별 무위험 수익률
    ...
    def _determine_action(self, pattern_signals, sentiment_signals, price_change, current_position, current_value, date):
        """
        거래 신호를 기반으로 행동 결정
        ...
        Returns:
            int: 액션 (0: 청산, 1: 매수, 2: 매도)
        """
        try:
            # 기본적으로 _get_advanced_action 메서드를 호출
            if hasattr(self, '_get_advanced_action'):
                return self._get_advanced_action(
                    pattern_signals, 
                    sentiment_signals, 
                    price_change, 
                    current_position, 
            ...
            # _get_advanced_action 메서드가 없으면 간단한 규칙 기반 접근
            else:
                # 패턴 신호와 감성 신호의 가중 평균
                pattern_weight = 0.5
                sentiment_weight = 0.3
                ...
                if isinstance(pattern_signals, dict):
                    pattern_signal = pattern_signals.get('score', 0)
                else:
                    pattern_signal = float(pattern_signals) if pattern_signals else 0
                    
                if isinstance(sentiment_signals, dict):
                    sentiment_signal = sentiment_signals.get('score', 0)
                else:
                    sentiment_signal = float(sentiment_signals) if sentiment_signals else 0
            ...
            self.logger.error(f"행동 결정 중 오류 발생: {str(e)}")
            # 오류 시 기본 행동 (홀딩)
            return 0
